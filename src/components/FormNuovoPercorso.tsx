...
function aggiungiMesi(dataStr: string, mesi: number): string {
  if (!dataStr) return '';
  const [anno, mese, giorno] = dataStr.split('-').map(Number);
  const targetDate = new Date(anno, mese - 1 + mesi, giorno);
  const a = targetDate.getFullYear();
  const m = String(targetDate.getMonth() + 1).padStart(2, '0');
  const g = String(targetDate.getDate()).padStart(2, '0');
  return `${a}-${m}-${g}`;
}
...
export function FormNuovoPercorso({
  ...
  const [durataMesiSelezionata, setDurataMesiSelezionata] = useState<number | null>(null);
  ...
  useEffect(() => {
    async function carica() {
      try {
        setLoading(true);
        const [c, s, p] = await Promise.all([getClienti(), getServizi(), getProdotti()]);
        setClienti(c);
        setServizi(s);
        setProdotti(p);
      } catch (err: any) {
  ...
  function cambiaDataInizio(nuovaData: string) {
    setDataInizio(nuovaData);
    if (durataMesiSelezionata) {
      setDataFine(aggiungiMesi(nuovaData, durataMesiSelezionata));
    }
  }

  function applicaDurata(mesi: number) {
    setDurataMesiSelezionata(mesi);
    if (dataInizio) {
      setDataFine(aggiungiMesi(dataInizio, mesi));
    }
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const percorso = await creaPercorso({
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                <Input
                  value={nome}
                  onChange={setNome}
                  placeholder="es. Percorso Tricologico Personalizzato"
                />
              </div>

              <div>
                <Label required>Data Inizio</Label>
                <Input type="date" value={dataInizio} onChange={cambiaDataInizio} />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[10px] text-apple-blue font-semibold">
                    (modificabile a mano)
                  </span>
                </div>
              ...
              <div className="sm:col-span-2 pt-1">
                ...
                <div className="flex flex-wrap gap-2">
                  {[3, 6, 12, 18, 24, 36, 48, 60].map((mesi) => (
                    <button
                      key={mesi}
                      type="button"
                      onClick={() => applicaDurata(mesi)}
                      className={`px-3 py-1.5 rounded-apple text-xs font-bold border transition-all ${
                        durataMesiSelezionata === mesi
                          ? 'bg-apple-blue text-white border-apple-blue shadow-sm scale-105'
                          : 'bg-gray-50 text-apple-darkgray border-gray-200 hover:bg-gray-100'
                      }`}
                    >
                      {mesi} mesi
                    </button>
